...
def resetRequests():
    while(1):
        ...
        while(currentCount>mustCount):
            ...
            subprocess.run(["docker", "exec", str(ports[p]), "./stop.sh"])
            subprocess.run(["docker","stop",str(ports[p])])
            subprocess.run(["docker", "rm", str(ports[p])])
            ports[p]=""
            print("exec")
            used ={k:v for k,v in ports.items() if len(ports[k])>0}
            currentCount=len(used.keys())
        ...
        while(currentCount<mustCount):
            ...
            subprocess.run(["docker", "run","-d","-p",str(p)+":"+str(3000),"--network","mynet","--entrypoint=./acts.sh","--name="+newc,"nandosnv/finalact"])
...
def cleanSlate():
    ...
    while(currentCount>mustCount):
        ...
        #exec("docker rm "+ports[p])
        print("REMOVING CONTAINER")
        subprocess.run(["docker", "exec", "act"+str(ports[p]), "./stop.sh"])
        subprocess.run(["docker","stop",str(ports[p])])
        subprocess.run(["docker", "rm", str(ports[p])])
        ports[p]=""
        print("exec")
