...
conn = mysql.connect()
cursor =conn.cursor()
cursor.execute('update crash set cflag=1 ;')
conn.commit()
conn.close()
...
def userposts():
    ...
    conn = mysql.connect()
    cursor=conn.cursor()
    cursor.execute('select cflag from crash;')
    conn.commit()
    CFLAG=cursor.fetchone()
    if CFLAG[0]==0:
        return jsonify({}),500
    conn.close()
...
def health_check():
    ...
    try:
        conn=mysql.connect()
        cursor=conn.cursor()
        cursor.execute('Select * from t_act ;')
...
def crash_server():
    ...
    if CFLAG[0]==0:
        ...
        cursor.execute('update crash set cflag = 0 ;')
        conn.commit()
        return jsonify({}),200
